...
class CyclePlugin(SpecEditorPlugin):
    ...
    def on_run(
        ...
        cycle mode:       ingest logs → detect bugs → push to Redis → PM Agent consumes
        cycle-graph mode: full PM Agent workflow (fix → test → deploy)
        coding mode:      coding agent only
        team mode:        all agents
        """
        # ── License gate: all cycle modes require Pro ──
        if mode in ("cycle", "cycle-graph", "coding", "team"):
            if not self._check_pro_license(project_path, settings):
                return False  # License check failed — abort

        if mode == "cycle":
            # Ingest mode: just read logs, find bugs, push to Redis
            return self._run_ingest(project_path, storage)
        if mode in ("cycle-graph", "team"):
            return self._run_pm_agent_workflow(project_path, storage, settings)
        if mode == "coding":
            return self._run_coding(project_path, storage, method, agents_config, settings, initial_task)
        return False
